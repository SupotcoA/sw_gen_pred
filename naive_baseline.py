...
def baseline_copy_mse_loss2(reshaped_data, mask, s,last=None):
    """
    计算基线模型的MSE loss（使用重构后的数据）
    ...
    # 基线预测: 用每个特征最后4个时间点的均值预测接下来s个时间点
    # 从输入中提取每个特征最后4个时间点的值
    # 由于重构后的数据中，每个特征的时间序列被展开，我们需要提取每个特征块的最后一个值
    ...
    # 将输入数据重塑为 [T-1, D, s]
    inputs_3d = inputs.reshape(T-1, D, s)  # [T-1, D, s]
    last_values = inputs_3d[:, :, -last:].mean(dim=-1)      # [T-1, D] 每个特征的最后4个值平均
    ...
    # 检查输入中每个特征最后一个值是否缺失
    input_mask_3d = input_mask.reshape(T-1, D, s)  # [T-1, D, s]
    last_values_mask = input_mask_3d[:, :, -last:].all(dim=-1)      # [T-1, D] 每个特征最后4个值的掩码
...
if __name__ == "__main__":
    ...
    # print(loss.mean()) #0.0474
    # print(loss[3:7].mean()) # 0.0208
    # print(loss)
    # # [0.0788, 0.0835, 0.1601, 0.0301, 0.0243, 0.0257, 0.0032, 0.0174, 0.0031]
